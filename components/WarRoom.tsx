...
// Helper: build a printable window with current strategy and memo
const exportStrategyToPdf = (opts: {
  memorandum: string;
  theoryTitle?: string;
  ...
  const { memorandum, theoryTitle, guidingPrinciple, tacticalApplication, opponentProfile } = opts;
  ...
  const html = `
    ...
    <html lang="en">
      <head>
        <meta charset="utf-8" />
        <title>JusticeAlly – Case Strategy Report</title>
        <style>
      ...
      <body>
        <div class="header">
           ...
           <div class="header-text">
             <h1>Case Strategy Report</h1>
             <p>JusticeAlly • Confidential Work Product • ${dateStr}</p>
           </div>
        </div>

        <div class="section">
          <h2>Counsel's Memorandum</h2>
          <pre>${memorandum || "No memorandum recorded."}</pre>
        </div>
        ...
        <div class="section">
          ...
          <h3>Case Theory</h3>
          <p><strong>"${theoryTitle || "No theory selected."}"</strong></p>
          
          <h3>Guiding Principle</h3>
          <blockquote>${guidingPrinciple || "—"}</blockquote>
          
          <h3>Tactical Application</h3>
          <p>${tacticalApplication || "—"}</p>

          ${opponentProfile ? `<h3>Opponent Profile</h3><p>${opponentProfile}</p>` : ''}
        </div>

        <div class="footer">
          Privileged & Confidential • Generated by JusticeAlly
        </div>
      </body>
    </html>
  `;
  ...
  // Wait for content to fully load (font rendering, etc) before printing
  win.onload = () => {
    win.focus();
    win.print();
  };
...
const WarRoom: React.FC<WarRoomProps> = ({ caseData, onFilesAdded, onLinkAdded, onNotesChange, onLoadDemo }) => {
  ...
  const handleExportPdf = () => {
      exportStrategyToPdf({
        memorandum: caseData?.notes || '',
        theoryTitle: caseData?.strategy?.sunTzu?.strategyName,
        guidingPrinciple: caseData?.strategy?.sunTzu?.quote,
        tacticalApplication: caseData?.strategy?.sunTzu?.application,
        opponentProfile: caseData?.strategy?.sunTzu?.opponentProfile
      });
  };
