...
import { SessionRecord, CaseData } from '../types';
...
const LiveSession: React.FC<LiveSessionProps> = ({ caseData }) => {
...
CASE SUMMARY: ${caseData.caseSummary || 'None available'}

COUNSEL NOTES: ${caseData.notes || 'None recorded'}

EVIDENCE VAULT (${caseData.documents?.length || 0} items):
${caseData.documents?.map((d, i) => `[${i + 1}] ${d.fileName} (Relevance: ${d.relevanceScore}/10) - ${d.summary}`).join('\n') || 'No documents'}

STRATEGY ANALYSIS:
- Case Theory: "${caseData.strategy?.sunTzu?.strategyName || 'Not yet developed'}"
- Sun Tzu Principle: "${caseData.strategy?.sunTzu?.quote || 'N/A'}"
- Tactical Application: ${caseData.strategy?.sunTzu?.application || 'N/A'}
- Opponent Profile: ${caseData.strategy?.sunTzu?.opponentProfile || 'N/A'}

CLAIM ELEMENTS ANALYSIS:
${caseData.strategy?.blackLetter?.claimElements?.map(el => `- ${el.element}: ${el.status.toUpperCase()} (Evidence: ${el.evidence || 'None'})`).join('\n') || 'Not analyzed'}

PROCEDURAL DEADLINES:
${caseData.strategy?.procedural?.deadlines?.join('\n') || 'None set'}

NEXT IMMEDIATE STEP: ${caseData.strategy?.procedural?.nextStep || 'Not determined'}
            ` : 'No case data available yet. This is a fresh consultation.';

            await clientRef.current?.connect(language, caseContext);
            setIsActive(true);
        }
    };
                        ...
                        <div className="z-10 text-center space-y-12 max-w-lg w-full">
                            ...
                            <div className="space-y-6">
                                ...
                                <button
                                    ...
                                    className={`w-full py-5 rounded-sm font-bold text-sm uppercase tracking-[0.2em] transition-all shadow-xl border ${isActive
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-slate-950">
                    ...
                    {transcript.map((msg, idx) => (
                        <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                            ...
                            <div className={`p-3 rounded-sm text-sm font-serif max-w-[90%] leading-relaxed ${msg.role === 'user'
